...
class SemaExplorerSCDFS(SemaExplorer):
    def __init__(
        self,
        simgr,
        max_length,
        ...
        super(SemaExplorerSCDFS, self).__init__(
            simgr,
            max_length,
            exp_dir,
            nameFileShort,
        ...
        self.log = logging.getLogger("ToolChainExplorerSCDFS")
    ...
    def step(self, simgr, stash="active", **kwargs):
        ...
        if (len(self.fork_stack) > 0 or 
            len(simgr.deadended) > self.deadended or 
            len(simgr.errored) > self.errored
        ):
            self.flag = True
        ...
        if self.flag or len(simgr.active) == 0:
            ...
            if len(simgr.stashes["pause"]) > 0 and len(simgr.active) == 0:
